        ...
        if(this.state.applied) {
            ...
            <div>
                ...
                <div style={{margin: '35px 15% 45px 15%'}}>
                    <div>
                        <h3>Position: {this.props.project.name}</h3>
                        <h3 style={{marginTop:'-10px'}}>Location: {this.props.project.location}</h3>
                        <div style={{marginTop: '40px'}}>
                            <p style={{fontSize:'15px', lineHeight: '1.6'}}>{this.props.project.description.split('\n\n').map((el, i) => <div style={{marginBottom: '20px'}} key={i}>{el}</div>)}</p>
                        </div>
                    </div>
                </div>
                <h1 style={{ marginTop: '80px', textAlign: 'center'}}>Apply</h1>
                <CandidateFormOpen
